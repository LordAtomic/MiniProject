...
<div class="container">
    ...
    if (isset($_POST["submit"])) {
        ...
        include("connect.php");
        $emailvrfy = "SELECT * FROM register WHERE email='$email' LIMIT 1";
        $result = mysqli_query($con, $emailvrfy);
        if (mysqli_num_rows($result) > 0) {
            array_push($error, 'Email already exists'); //To check multiple emails on databse if their is 1 or more it  not resgister this
        }
        if (count($error) > 0) {
            ...
            $sql = "INSERT INTO register (first, last, email, password, status) VALUES ('$firstname', '$lastname', '$email', '$hashed_password', 'approved')";
